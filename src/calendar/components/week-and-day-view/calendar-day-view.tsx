import { CalendarDays, Clock, Square, User } from "lucide-react";
        ...
        <div>
          ...
          <div className="relative z-20 flex border-b">
            ...
            <span className="flex-1 border-l py-2 text-center text-xs font-medium text-muted-foreground">
              {format(selectedDate, "EEEE", {locale: hu}).charAt(0).toUpperCase() + format(selectedDate, "EEEE", {locale: hu}).slice(1)} <span className="font-semibold text-foreground">{format(selectedDate, "d")}</span>
            </span>
          </div>
        </div>
              ...
              {hours.map((hour, index) => (
                <div key={hour} className="relative" style={{ height: "96px" }}>
                  <div className="absolute -top-3 right-2 flex h-6 items-center">
                    {index !== 0 && <span className="text-xs text-muted-foreground">{format(new Date().setHours(hour, 0, 0, 0), "HH:mm", {locale: hu})}</span>}
                  </div>
                </div>
              ))}
            </div>
                  ...
                  return (
                    <div key={hour} className={cn("relative", isDisabled && "bg-calendar-disabled-hour")} style={{ height: "96px" }}>
                      ...
                      <DroppableTimeBlock date={selectedDate} hour={hour} minute={0}>
                        <AddEventDialog startDate={new Date( selectedDate.getFullYear(), selectedDate.getMonth(), selectedDate.getDate(), hour, 0 )}>
                          <div className="absolute inset-x-0 top-0 h-[24px] cursor-pointer transition-colors hover:bg-accent" />
                        </AddEventDialog>
                      </DroppableTimeBlock>

                      <DroppableTimeBlock date={selectedDate} hour={hour} minute={15}>
                        <AddEventDialog startDate={new Date( selectedDate.getFullYear(), selectedDate.getMonth(), selectedDate.getDate(), hour, 15 )}>
                      ...
                      <DroppableTimeBlock date={selectedDate} hour={hour} minute={30}>
                        <AddEventDialog startDate={new Date( selectedDate.getFullYear(), selectedDate.getMonth(), selectedDate.getDate(), hour, 30 )}>
                          <div className="absolute inset-x-0 top-[48px] h-[24px] cursor-pointer transition-colors hover:bg-accent" />
                        </AddEventDialog>
                      </DroppableTimeBlock>

                      <DroppableTimeBlock date={selectedDate} hour={hour} minute={45}>
                        <AddEventDialog startDate={new Date( selectedDate.getFullYear(), selectedDate.getMonth(), selectedDate.getDate(), hour, 45 )}>
        ...
        <div className="flex-1 space-y-6">
          {currentEvents.length > 0 ? (
            <div className="flex items-start gap-2 px-4 pt-4">
              ...
              <p className="text-sm font-semibold text-foreground">Jelenleg folyamatban</p>
            </div>
          ) : (
            <p className="p-4 text-center text-sm italic text-muted-foreground">Nincs folyamatban semmilyen tevékenység.</p>
          )}
                  ...
                  return (
                    <div key={event.id} className="space-y-1.5">
                      <div className="flex items-center gap-1.5">
                        <svg width="8" height="8" viewBox="0 0 8 8" className="event-dot shrink-0" fill={event.color}>
                      ...
                      <div className="flex items-center gap-1.5 text-muted-foreground">
                        ...
                        <span className="text-sm">{format(new Date(event.startDate), "yyyy. MMM d.", { locale: hu })}</span>
                      </div>

                      <div className="flex items-center gap-1.5 text-muted-foreground">
                        <Clock className="size-3.5" />
                        <span className="text-sm">
                          {format(parseISO(event.startDate), "HH:mm", { locale: hu })} –{" "}
                          {format(parseISO(event.endDate), "HH:mm", { locale: hu })}
                        </span>
                      </div>
                      <Separator />
                    </div>
