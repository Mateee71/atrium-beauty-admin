...
export function YearViewMonth({ month, events, onViewChange }: IProps) {
  const { push } = useRouter();
  const { setSelectedDate } = useCalendar();

  const monthName = format(month, "MMMM", { locale: hu }).charAt(0).toUpperCase() + format(month, "MMMM", { locale: hu }).slice(1);
  ...
  return (
    <div className="flex flex-col">
      <button
        type="button"
        onClick={handleClick}
        className="w-full rounded-t-lg border px-3 py-2 text-sm font-semibold hover:bg-accent focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring cursor-pointer"
      >
        {monthName}
      </button>
      ...
      <div className="flex-1 space-y-2 rounded-b-lg border border-t-0 p-3">
        ...
        <div className="grid grid-cols-7 gap-x-0.5 gap-y-2">
          {daysInMonth.map((day, index) => {
            ...
            return <YearViewDayCell key={`day-${day}`} day={day} date={date} events={dayEvents} onViewChange={onViewChange} />;
          })}
        </div>
      </div>
    </div>
