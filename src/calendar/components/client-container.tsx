...
export function ClientContainer({ view, onViewChange }: IProps) {
  ...
  }, [selectedDate, selectedUserId, safeEvents, view])

  const singleDayEvents = filteredEvents.filter(e => {
    const s = parseISO(e.startDate)
    const eD = parseISO(e.endDate)
    return isSameDay(s, eD)
  })
  ...
  const eventStartDates = useMemo(() => {
    return filteredEvents.map(e => ({ ...e, endDate: e.startDate }))
  }, [filteredEvents])
