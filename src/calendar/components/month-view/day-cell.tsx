      ...
      <div className={cn("flex h-full flex-col gap-1 border-l border-t py-1.5 lg:py-2", isSunday && "border-l-0")}>
        <span
          className={cn(
            ...
            isToday(date) && "flex w-6 translate-x-1 items-center justify-center rounded-full bg-primary px-0 font-bold text-white"
          )}
        >
          {day}
        </span>
        ...
        {cellEvents.length > MAX_VISIBLE_EVENTS && (
          <p className={cn("h-4.5 px-1.5 text-xs font-semibold text-muted-foreground", !currentMonth && "opacity-50")}>
            ...
            <span className="hidden sm:inline"> {cellEvents.length - MAX_VISIBLE_EVENTS} további...</span>
          </p>
        )}
      </div>
    </DroppableDayCell>
